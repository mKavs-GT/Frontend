                ...
                <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="flex flex-col gap-6">
                  {project.sprints.filter(s => s._id === activeSprintMap[project._id]).map(sprint => (
                    <div key={sprint._id} className="bg-white dark:bg-zinc-950/20 rounded-[2.5rem] p-8 border border-zinc-200 dark:border-zinc-800/20 shadow-sm">
                      ...
                      <div className="flex flex-col lg:flex-row gap-6 overflow-x-auto pb-6 hide-scrollbar">
                        {Object.entries(COLUMN_TITLES).map(([key, title]) => (
                          <div key={key} className="flex-shrink-0 w-full lg:w-[320px] bg-zinc-50/50 dark:bg-zinc-900/30 rounded-[2.5rem] p-6 border border-zinc-200/50 dark:border-zinc-800/30 flex flex-col gap-5">
                            ...
                            <div className="flex flex-col gap-4 min-h-[150px]">
                                {sortTasks(sprint.columns[key] || []).map(task => (
                                  <div key={task.id} className="bg-white dark:bg-zinc-900 p-5 rounded-[2rem] shadow-md border border-zinc-200/50 dark:border-zinc-800/50 flex flex-col min-h-[180px] transition-all hover:border-indigo-500/30 group">
                                    {/* Header: Title + Priority */}
                                    <div className="flex items-start justify-between gap-3 mb-4">
                                      <p className="text-[13px] font-bold text-zinc-900 dark:text-white leading-tight flex-1 line-clamp-2 overflow-hidden overflow-ellipsis" style={{ display: '-webkit-box', WebkitLineClamp: '2', WebkitBoxOrient: 'vertical' }}>
                                        {task.content}
                                      </p>
                                      <span className={`shrink-0 text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${
                                    ...
                                    <div className="flex flex-col gap-3 mb-6">
                                      {key === 'allTasks' && task.assignees.some(a => a.userId === user.email || a.userId === user.uid) && (
                                        <button onClick={() => moveTask(project._id, sprint._id, task.id, 'allTasks', 'ongoing')} className="w-full py-3 rounded-xl bg-indigo-600 text-white text-[10px] font-black uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-500/20 active:scale-95">Accept Task</button>
                                      )}
                                      
                                      {key === 'ongoing' && task.assignees.some(a => a.userId === user.email || a.userId === user.uid) && (
                                        <div className="flex items-center justify-center py-2 bg-indigo-50/50 dark:bg-indigo-500/5 rounded-xl border border-indigo-100/50 dark:border-indigo-500/10">
                                          <input 
                                            type="checkbox" 
                                      ...
                                      {key === 'testing' && user.email === 'agent05mrm@gmail.com' && (
                                        <label className="flex items-center justify-center gap-3 cursor-pointer group/check w-full py-3 px-3 rounded-xl bg-amber-50 dark:bg-amber-500/5 border border-amber-200/50 dark:border-amber-500/20 hover:bg-amber-100/50 transition-all">
                                          ...
                                          <span className="text-[9px] font-black text-amber-700 dark:text-amber-400 uppercase tracking-widest whitespace-nowrap">Testing Complete</span>
                                        </label>
                                      )}
                                      
                                      {key === 'approval' && user.email === 'agent01mrk@gmail.com' && (
                                        <label className="flex items-center justify-center gap-3 cursor-pointer group/check w-full py-3 px-3 rounded-xl bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200/50 dark:border-emerald-500/20 hover:bg-emerald-100/50 transition-all">
                                          ...
                                          <span className="text-[9px] font-black text-emerald-700 dark:text-emerald-400 uppercase tracking-widest whitespace-nowrap">Approve & Live</span>
                                        </label>
                                      )}
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800/50">
                                      <div className="flex -space-x-2">
                                        {task.assignees.map((a, i) => (
                                          <img key={i} src={a.avatar} alt={a.name} title={a.name} className="w-8 h-8 rounded-full border-2 border-white dark:border-zinc-900 shadow-sm" />
                                        ))}
                                      </div>
                                      ...
                                      <div className="flex items-center gap-1">
                                        {user?.isExecutive && !['approval', 'live'].includes(key) && (
                                          ...
                                          }} className="p-2 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-400 hover:text-indigo-600 transition-colors">
